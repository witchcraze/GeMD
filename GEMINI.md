...
2.  **AIの対応:**
    ...
    *   理解した内容に基づき、実装や仕様変更に伴って更新が必要なドキュメントを特定します。
    *   実現に必要なタスク（コード変更、ドキュメント更新など）を分解して、具体的な実装計画を立てます。
    *   以下のフォーマットで、実装計画をIssueにコメントします。その際、本文の長短に関わらず、必ず一時ファイルを作成し`--body-file`オプションを使用します。
        ...
        - `docs/03_TESTING_GUIDELINES.md`: (更新内容の要約)
...
2.  **AIの対応:**
    ...
    *   PRの本文には、関連��るIssueへのリンク（例: `Closes #12`）を必ず含めます。

#### **ステップ4: セルフレビューとPRのレビュー依頼**
1.  **トリガー:** Pull Requestが作成される。
2.  **AIの対応:**
    *   Issueの `status: implementing` ラベルを削除し、`status: review` ラベルを付与します。
    *   PR上で、AI自身によるセルフレビューを実施します。以下の観点で確認し、その結果をPRにコメントします。その際、本文の長短に関わらず、必ず一時ファイルを作成し`--body-file`オプションを使用します。
        ```markdown
        ### セルフレビュー報告

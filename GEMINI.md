...
*   **単一責任の原則:** 一つのIssueは、一つの関心事（機能追加、バグ修正、��ファクタリングなど）に集中します。複数の目的を一つのIssueに混在させません。
...
2.  **AIの対応:**
    ...
    *   実現��必要なタスク（コード変更、ドキュメント更新など）を分解し、変更予定のファイルをすべてリストアップした、具体的な実装計画を立てます。
    *   以下のフォーマットで、実装計画をIssueにコメントした後、「GitHub Issue上で計画をレビューし、『承認』とコメントするか、このCLI上で承認を伝えてください。**承認後、次のステップに進むよう、私に指示をお願いします。**」とユーザーに伝え、応答を待ちます。
        ...
        **変更対象ファイル:**
        ...
        - `path/to/file2.ext`: (このファイルに対する具体的な変更内容を記述��ます)
...
2.  **AIの対応:**
    ...
    *   実装計画に沿って、コードの変更、ファイルの作成・編集、テストの追加などを行います。
        *   **【重要】計画外のファイル変更の原則禁止:** AIは、実装計画で合意されたファイル以外は原則として変更しません。も���実装の過程で、計画外のファイル変更が必要だと判断した場合は、作業を中断し、その理由と変更内容をユーザーに報告し、承認を求めます。
...
2.  **AIの対応:**
    ...
    *   **セルフレビューの実施:** 以下の観点でセルフレビューを実施します。
        ...
        3.  **`GEMINI.md`の規約（テスト、命名規則など）を遵��しているか？**
    ...
    *   **レビュー依頼:** 自己修正が不要、または完了した場合、以下のフォーマットでセルフレビューの結果をPRにコメントした後、「GitHub PR上でレビューし、『マージを承認します』とコメントするか、このCLI上で承認を伝えてください。**承認後、マージを実行するよう、私に指��をお願いします。**」とユーザーに伝え、応答を待ちます。
        ```markdown
        ### セルフレビュー報告
...
*   **手順:**
    ...
    3.  ユーザーの承認��、AIはコードの変更と合わせてドキュメントの更新も行います。

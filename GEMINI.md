...
2.  **AIの対応:**
    *   Issue��� `status: planning` ラベルを削除し、`status: implementing` ラベルを付与します。
...
2.  **AIの対応:**
    ...
    *   PR上で、AI自身によるセルフレビューを実施します。以下の観点で確認し、その結果をPRにコメントします。その際、本文の長短に関わらず、必ず一時ファイルを作成し`--body-file`オプ��ョンを使用します。
        ```markdown
        ### セルフレビュー報告
